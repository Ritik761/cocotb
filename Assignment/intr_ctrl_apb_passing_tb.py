...
async def test_2_apb_irq_data_race(dut):
    """
    APB write transaction and interrupt source update happen concurrently.
    Verify APB transaction is not corrupted when ISR changes
    while APB transfer is in progress.
    """

    cocotb.start_soon(Clock(dut.clk, 10, units="ns").start())
    await reset_dut(dut)

    # Slow slave so APB remains busy for a few cycles
    ...
    # APB should now be starting

    await RisingEdge(dut.clk)

    # While APB transaction is still active,
    # update interrupt source
    ...
    # At minimum IRQ0 must not be lost
    assert (isr_val & 0x01), \
        f"T2 FAIL: IRQ0 lost after race condition. ISR=0x{isr_val:02X}"

    # Verify APB write actually happened
    ...
    dut._log.info(
        f"T2 PASS: Race handled correctly. "
        f"APB_DATA=0x{txn['data']:02X}, ISR=0x{isr_val:02X}"
    )
...
async def test_3_priority_encoder_correctness(dut):
    """
    Incorrect ISR due to scoreboard priority encoder modeling.
    Verify that when multiple interrupts occur simultaneously,
    the highest-priority interrupt (IRQ0) is selected.
    """
    ...
    # IRQ0 + IRQ2 + IRQ5
    # Expected highest priority = IRQ0
    # --------------------------------------------------
    ...
    # Interrupt output should assert
    assert int(dut.INT.value) == 1, \
        "T3 FAIL: INT output not asserted"

    # Highest priority should be IRQ0
    vect = int(dut.INT_VECT.value)

    assert vect == 0, \
        f"T3 FAIL: Expected IRQ0, got IRQ{vect}"

    # Read ISR
    isr = await cpu_read(dut, ADDR_ISR)

    assert (isr & 0x01), \
        f"T3 FAIL: ISR does not indicate IRQ0, ISR=0x{isr:02X}"

    dut._log.info(
        f"T3 PASS: Highest priority interrupt selected correctly. "
        f"INT_VECT={vect}, ISR=0x{isr:02X}"
    )
...
async def test_4_delayed_apb_response(dut):
    """
    Faulty handling of delayed APB response.
    Verify that the DUT correctly waits for a delayed
    PREADY response and completes the APB transaction.
    """
    ...
    # During delay APB should remain busy
    for _ in range(5):
        await RisingEdge(dut.clk)
    ...
    assert busy == 1, \
        "T4 FAIL: apb_busy should stay high during delayed response"

    dut._log.info("APB busy asserted during delayed response")
    ...
    assert slave.transaction_count() == 1, \
        f"T4 FAIL: Expected 1 APB transaction, got {slave.transaction_count()}"
    ...
    assert int(dut.apb_busy.value) == 0, \
        "T4 FAIL: apb_busy should be 0 after transaction completion"
...
async def test_5_swir_hw_irq_race(dut):
    ...
    # Verify SW interrupt exists
    assert ((ipr | isr) & 0x08), \
        f"T5 FAIL: SW IRQ3 lost (IPR=0x{ipr:02X}, ISR=0x{isr:02X})"

    # Verify HW interrupt exists
    assert ((ipr | isr) & 0x01), \
        f"T5 FAIL: HW IRQ0 lost (IPR=0x{ipr:02X}, ISR=0x{isr:02X})"

    dut._log.info(
        f"T5 PASS: HW IRQ0 and SW IRQ3 both preserved "
        f"(IPR=0x{ipr:02X}, ISR=0x{isr:02X})"
    )
...
async def test_6_interrupt_message_data_correctness(dut):
    """
    Verify APB write data matches the actual ISR value.
    """
    ...
    # Main specification check
    assert apb_data == isr_val, \
        f"T6 FAIL: APB data=0x{apb_data:02X} ISR=0x{isr_val:02X}"

    # IRQ3 sanity check
    assert isr_val == 0x08, \
        f"T6 FAIL: Expected ISR=0x08 got 0x{isr_val:02X}"

    dut._log.info(
        f"T6 PASS: APB message data matches ISR (0x{isr_val:02X})"
    )
...
async def test_7_level_interrupt_ipr_sync(dut):
    """
    Stray Level Interrupt Signal
    Verify IPR correctly tracks a level interrupt
    and retains the pending interrupt until CPU ACK.
    """
    ...
    dut._log.info(f"IPR while IRQ0 asserted = 0x{ipr_set:02X}")
    ...
    dut._log.info(f"IPR after IRQ0 removed = 0x{ipr_pending:02X}")

    # Pending interrupt should still be recorded
    assert (ipr_pending & 0x01), \
        f"T7 FAIL: Pending interrupt lost unexpectedly, IPR=0x{ipr_pending:02X}"
    ...
    dut._log.info(f"IPR after ACK = 0x{ipr_after_ack:02X}")
    ...
    dut._log.info(
        ...
        f"(Set=0x{ipr_set:02X}, Pending=0x{ipr_pending:02X}, Cleared=0x{ipr_after_ack:02X})"
    )
...
async def test_8_apb_error_response_handling(dut):
    ...
    # Check APB error status
    micr = await cpu_read(dut, ADDR_MICR)

    dut._log.info(f"MICR = 0x{micr:02X}")

    assert ((micr >> 1) & 1) == 1, \
        f"T8 FAIL: apb_err not set after PSLVERR, MICR=0x{micr:02X}"

    dut._log.info(
        f"T8 PASS: APB error correctly captured, MICR=0x{micr:02X}"
    )
...
async def test_9_masked_level_irq_becomes_pending_after_unmask(dut):
    """ 
    Scoreboard prediction for level interrupt when IMR bit is cleared.
    IRQ0 is already active while masked.
    After unmasking, controller must detect it immediately.
    """
    ...
    # Keep IRQ0 asserted while masked
    dut.irq.value = 0x01

    for _ in range(3):
        await RisingEdge(dut.clk)

    # While masked, IPR should not show pending interrupt
    ipr_masked = await cpu_read(dut, ADDR_IPR)

    dut._log.info(
        f"IPR while masked = 0x{ipr_masked:02X}"
    )
    ...
    # Interrupt should now become pending
    ipr_unmasked = await cpu_read(dut, ADDR_IPR)

    dut._log.info(
        f"IPR after unmask = 0x{ipr_unmasked:02X}"
    )

    assert (ipr_unmasked & 0x01), \
        f"T9 FAIL: Pending level IRQ not detected after unmask, IPR=0x{ipr_unmasked:02X}"

    # In line mode INT should assert
    assert int(dut.INT.value) == 1, \
        "T9 FAIL: INT not asserted after unmasking active level IRQ"

    dut._log.info(
        f"T9 PASS: Active masked IRQ correctly became pending after unmask, IPR=0x{ipr_unmasked:02X}"
    )
